...
ConnectionManager.prototype.send = function(host, port, message, callback) {
    var self = this;
    ...
    if (connection) {
        ...
        connection.write(JSON.stringify(message), callback);
    } else {
        this.connect(host, port, function() {
            self.send(host, port, message, callback);
        });
    }
};
